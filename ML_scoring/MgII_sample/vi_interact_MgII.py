...
def plot_box(someX, gradelab):
    ...
    pl.text(xcent, ycent-150, gradelab, color='red')
...
def plot_inter(line, fout):
    ...
    if os.path.exists(img):
        ...
        imag = pl.imread(img)
        pl.imshow(imag)
        plot_box((1.+float(cols[3]))*2796.4, cols[4])
        pl.show()
    else:
    ...
    moveon = ['next', 'quit']
    print " "
    print "Inspecting grade %s Mg II doublet at z=%.3f (%i A)" % (cols[4], float(cols[3]), int((1.+float(cols[3]))*2796.4))
...
# set the input file
#filename = 'jhu/absorbers_not_recovered_by_jhu_notes_gradeD_copy.dat'
filename = 'randsubset_dr7_absorbers_in_dr4_notes.dat'
